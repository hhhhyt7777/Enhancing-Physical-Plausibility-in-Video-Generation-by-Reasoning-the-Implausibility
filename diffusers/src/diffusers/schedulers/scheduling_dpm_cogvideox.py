...
class CogVideoXDPMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        ##
        # pc_grad, sa_grad = cond_fn
        # if pc_grad is not None and sa_grad is not None :
        #     # === 梯度裁剪 ===
        #     grad_clip_thresh = 30.0
        #     pc_grad = safe_grad_clip(pc_grad, grad_clip_thresh)
        #     sa_grad = safe_grad_clip(sa_grad, grad_clip_thresh)
        # 
        #     # === 时间归一化并反向映射，确保前期强、后期弱 ===
        #     t_frac = 1.0 - (timestep / self.config.num_train_timesteps)  # early:1, late:0
        #     t_frac = float(t_frac)
        #     gamma = 5.0  # 衰减指数，建议2~5
        #     s_base = 8.0  # 初始引导强度
        # 
        #     # 指数衰减引导系数（稳定）
        #     s_pc = s_base * (0.5 + 0.5 * np.exp(-gamma * t_frac))
        #     s_sa = s_base * (0.5 + 0.5 * np.exp(-gamma * t_frac))
        # 
        #     # sqrt_one_minus_alpha = (1 - alpha_prod_t).sqrt()
        #     sigma = ((1 - alpha_prod_t_prev) / (1 - alpha_prod_t)) * (1 - alpha_prod_t / alpha_prod_t_prev)
        # 
        #     # === 构造引导项 ===
        #     guidance_pc = s_pc * sigma * pc_grad
        #     guidance_sa = s_sa * sigma * sa_grad
        # 
        #     # === 梯度幅值保护，防止模糊或爆发 ===
        #     if guidance_pc.abs().max() < 1e-6 and guidance_sa.abs().max() < 1e-6:
        #         print(f"[Warn] Step {timestep}: Grad guidance too weak, skipping.")
        #     else:
        #         model_output = model_output - guidance_pc #- guidance_sa
        # 
        #     # === 调试输出 ===
        #     print(f"\n🌀 timestep = {timestep} | s_pc = {s_pc:.4f} | s_sa = {s_sa:.4f}")
        #     print(f"[PC ] mean={guidance_pc.mean():.4f}, std={guidance_pc.std():.4f}")
            # print(f"[SA ] mean={guidance_sa.mean():.4f}, std={guidance_sa.std():.4f}")
        ##
...
def safe_grad_clip(grad, max_norm=30.0):
    norm = grad.norm()
